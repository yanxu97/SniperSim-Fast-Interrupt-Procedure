...
#define MYLOG(...)                                                                                                                                           \
    {                                                                                                                                                        \
        ScopedLock l(iolock);                                                                                                                                \
        fflush(stderr);                                                                                                                                      \
        fprintf(stderr, "[%8lu] %dcor %-25s@%03u: ", getPerformanceModel()->getCycleCount(ShmemPerfModel::_USER_THREAD), m_core_id, __FUNCTION__, __LINE__); \
...
{
    std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: ModeledString" << std::endl;

    switch (modeled) {
    case Core::MEM_MODELED_NONE:
        return "none";
    case Core::MEM_MODELED_COUNT:
        return "count";
    case Core::MEM_MODELED_COUNT_TLBTIME:
        return "count/tlb";
    case Core::MEM_MODELED_TIME:
        return "time";
    case Core::MEM_MODELED_FENCED:
        return "fenced";
    case Core::MEM_MODELED_RETURN:
        return "return";
    }
    return "?";
}
...
{
    std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: Core" << std::endl;
...
bool Core::countInstructions(IntPtr address, UInt32 count)
{
    // too many times being called
    // std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: countInstructions" << std::endl;
    bool check_rescheduled = false;
    // PerformanceModel* prfmdl = getPerformanceModel();

    m_instructions += count;
    // std::cerr << "+++ : " << m_instructions << std::endl;
    // std::cerr << "--- : " << m_performance_model->getInstructionCount() << std::endl;
    if (m_bbv.sample())
...
void Core::hookPeriodicInsCheck()
{
    // std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: hookPeriodicInsCheck" << std::endl;
...
void Core::hookPeriodicInsCall()
{
    // std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: hookPeriodicInsCall" << std::endl;
...
bool Core::accessBranchPredictor(IntPtr eip, bool taken, bool indirect, IntPtr target)
{
    // too many times
    std::cerr << "[IAN's TESTING] core.cc: CREEPY MOVEMENT: accessBranchPredictor" << std::endl;
    PerformanceModel* prfmdl = getPerformanceModel();
    BranchPredictor* bp = prfmdl->getBranchPredictor();
    std::cerr << "elapsed time from performance_model.cc is : " << prfmdl->getElapsedTime() << std::endl;
    std::cerr << "" << std::endl;
...
void Core::logMemoryHit(bool icache, mem_op_t mem_op_type, IntPtr address, MemModeled modeled, IntPtr eip)
{
...
void Core::emulateCpuid(UInt32 eax, UInt32 ecx, cpuid_result_t& res) const
{
    switch (eax) {
    case 0x0: {
        cpuid(0, 0, res);
